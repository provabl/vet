...
### Deep AMI-content scanning (`vet ami-scan`)

Beyond asserting an AMI's provenance/verdict, `vet ami-scan` inspects the image's **disk contents**:
it resolves the AMI's backing EBS snapshot, mounts a copy of it **read-only** on an operator-provided
helper instance, runs syft over the filesystem, and stores the SBOM where `vet verify --check-cves`
reads it. AMIs auto-route to the **distro-aware grype** scanner (a naive OSV `Linux` query is
false-clean on an Amazon Linux / RHEL AMI — grype matches the distro's ALAS/RHSA advisories).

```bash
# Prereqs: a running, SSM-managed, syft+awscli-equipped helper instance, and an
# S3 bucket the helper can PutObject + you can GetObject.
vet ami-scan ami-0abc123 --helper-instance i-0helper --az us-east-1a --scan-bucket my-vet-staging
vet verify   ami-0abc123 --check-cves high       # gate on the scanned SBOM (auto-routes to grype)
```

vet creates a volume from the snapshot and attaches it to the helper, then **detaches and deletes
that volume when done** — it never creates/terminates the helper instance and never touches the AMI's
own snapshot. The CVE source is pluggable (`--cve-source osv|grype`); Amazon Inspector is the other
viable source (see provabl/vet#32).
